...
export default function CategoryPage() {
  ...
  async function loadCategory() {
    try {
      setLoading(true);
      const data = await apiFetch(`/catalog/category/${slug}`);
      setWatchfaces(data || []);
      
      // Получаем название категории из первого элемента (если есть)
      if (data && data.length > 0 && data[0].categories) {
        const category = data[0].categories.find((c: any) => c.slug === slug);
        if (category) {
